...
class OpenType:
    ...
    TABLE_HEAD = multichar('head')          # TrueType table groups
...
def get_table_directory(data):
    ...
    for i in range(0, numTables):
        ...
        group, check, bongo, dirlen = struct.unpack(OpenType.TABLE_DIR_UNPACK, data[start:end])
        table_dir[group] = {'offset': bongo, 'length': dirlen, 'checksum': check}
